...
import { getThreadById, getThreadsForBuyer } from '@/lib/chat/storage';
...
export default function BuyerChatDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [threads, setThreads] = useState<ChatThread[]>(() => {
        ...
        const buyerThreads = getThreadsForBuyer(user.id);
        return buyerThreads.sort((a, b) => new Date(b.lastMessageAt).getTime() - new Date(a.lastMessageAt).getTime());
    });
    ...
    const isUnauthorized = thread && thread.buyerId !== user?.id;
    ...
    return (
        <ProtectedRoute allowedRoles={['buyer']}>
            <div className="min-h-screen bg-gray-50 py-12">
                <Container>
                    <Link href="/dashboard/buyer/chats" className="mb-6 inline-flex lg:hidden items-center gap-2 text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[700px]">
                        ...
                        <div className="lg:col-span-2 h-full">
                            <ChatWindow
                                ...
                                otherUserLabel={`Project Seller (ID: ${thread.sellerId})`}
                            />
                        </div>
                    </div>
                </Container>
